...
OutputDir=/home/groups/hpcbio_shared/azza/human_WES_multi_sample_synthetic/data/reads/error_rates
DatasetName=synthetic_var_error_rate

NumberOfChunks=1

NumberOfSamples=10

echo "Error rates of the files are": > $OutputDir/simulation_parameters.txt
echo "sample-name error-rate" > $OutputDir/simulation_parameters.txt
...
do
        ...
        random_number=$(awk -v i=$i 'BEGIN{srand(i*100); print (rand()*0.3) }') #i is the random number seed
	echo ${i} ${random_number} >> $OutputDir/simulation_parameters.txt
	NeatParams="-R 100 -c 5 -E $random_number -p 2 -t $targeted_region -v $known_variants --pe 300 30 --bam --vcf --rng 100 "
	...
	echo "-W depend=afterok:${AllChunksJobIds}" >> ${OutputLogsFolder}/${DatasetName}.MergeChunks.qsub
