...
setwd("/home/groups/hpcbio_shared/azza/H3A_NextGen_assessment_set3/results/p4_indelRealign_bqsr/bqsr/default")  
...
pdf('/home/groups/hpcbio_shared/azza/H3A_NextGen_assessment_set3/results/p4_indelRealign_bqsr/bqsr/bqsr_effect_detailed.pdf',title = 'Effect of parameter chages in the bqsr file')
  ...
  for (file in seq_along(before)){
    ...
    if (length(valuea) ){
     ...
     plot(qb$QualityScore,qb$EmpiricalQuality,pch=20,col='violetred',cex=.5,
          xlab='Reported Qualities', ylab='Empirical Qualities', xlim=c(0,70), ylim=c(0,70),
          main = paste('BQSR under \n', param_name[i-1],' = ', valueb ))
     points(qa$QualityScore,qa$EmpiricalQuality,pch=20,col='blue',
            cex=.5)
