...
cd $results/p2_alignment/
module load samtools/1.2
echo k Total_reads percentage_mapped Mean_MAPQ > alignment_summary_changing_k.txt
for i in $(seq 10 2 30); do
	total=$(grep total a$i.summary.txt | cut -d ' ' -f1)
	mapped=$(samtools view -c -F0x4 a$i.bam)
	mapq=$(samtools view a$i.bam | awk '{sum+=$5} END { print sum/NR}')
	echo $i $total $mapped $mapq >> alignment_summary_changing_k.txt
done
###########################################
#igv mAYBE
